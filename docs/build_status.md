...
Three GitHub Actions validation runs reached the Android build stage. Rust tests, Rust Android libraries, Flutter analysis, and Flutter tests passed. Version 1.0.2 migrated the generated Android app module to AGP 9 built-in Kotlin and the typed JVM 17 compiler options DSL. Version 1.0.3 fixes the subsequent Build Tools 36 AIDL failure by assigning an explicit ID to every method while retaining Shizuku's reserved destroy transaction ID.
